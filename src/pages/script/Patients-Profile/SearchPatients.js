...
import { FindMedicineReleasePerPatient, MedReleasePerPatient } from 'src/composables/Medicine'
...
export default {
  ...
  setup() {
    ...
    let downloadDisable = ref(typeof PatientsList.value === 'undefined' ? true : false)
    ...
    const searchPatients = () => {
      let payload = {
        search_by: {
          ...
          search_string: searchString.value === null ? '' : searchString.value.replace(/\s+/g, ''),
        },
    ...
    const checkPatientQueue = (patient_id, sex) => {
      CheckPatientQueue({ patient_id: patient_id, sex: sex }).then((response) => {
