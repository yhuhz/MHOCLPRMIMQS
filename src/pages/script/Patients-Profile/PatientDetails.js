...
export default {
  ...
  setup() {
    ...
    if (keySession && (keySession.department === 1 || keySession.department === 5)) {
    ...
    watch(
      ...
      () => {
        ...
        name.value =
          ...
          (PatientDetails.value.suffix === null ? '' : ' ' + PatientDetails.value.suffix)

        if (keySession && keySession.department === 5 && selectedDepartment.value === 'OPD') {
          ...
          (selectedDepartment.value === 'Prenatal' || selectedDepartment.value === 'Immunization')
        ) {
          isbtnDisabled.value = false
        }
      },
    ...
    if (selectedDepartment.value === 'Immunization') {
      ...
      if (
        keySession &&
        (keySession.department === departments.indexOf(selectedDepartment.value) + 1 ||
    ...
    const onRowClick = (evt, row) => {
      Loading.show()
      FindRecordDetails(row.record_id, selectedDepartment.value).then((response) => {
        Loading.hide()
      })
    ...
    const removeCurrentPatient = () => {
      Loading.show()
      RemovePatientFromQueue({ queue_id: route.params.queue }).then((response) => {
    ...
    if (route.params.queue) {
      ...
      GetRecords({
        ...
        if (keySession && keySession.department === 1 && response.data.length !== 0) {
    ...
    if (
      ...
      GetRecords({
        ...
        if (response.data.length > 0 && selectedDepartment.value === 'Prenatal') {
          let record = response.data[0]
          console.log('res', record)
          FindRecordDetails(record.record_id, selectedDepartment.value).then((response) => {
            console.log(response)
            Loading.hide()
