...
export default {
  ...
  setup() {
      ...
      () => {
        ...
        if (!route.params.queue) {
          ...
          selectedCheckup.value =
            RecordArrays.value.length !== 0 ? RecordArrays.value[0].checkup_date : null
        ...
        bmi.value =
          ...
          typeof prenatal_checkup.value.height === 'undefined'
            ? ''
            : (
                prenatal_checkup.value.weight / Math.pow(prenatal_checkup.value.height / 100, 2)
              ).toFixed(2)
    ...
    RecordArrays.value.forEach((r) => {
      if (r.checkup_date === selectedCheckup.value) {
        prenatal_checkup.value = r
        checkup_date_prenatal.value = prenatal_checkup.value.checkup_date.replaceAll('-', '/')
      }
    })
    ...
    const updateBMI = () => {
      bmi.value =
        ...
        (prenatal_checkup.value.weight / Math.pow(prenatal_checkup.value.height / 100, 2)).toFixed(
          2,
        ) > 0
          ? (
              prenatal_checkup.value.weight / Math.pow(prenatal_checkup.value.height / 100, 2)
    ...
    const editPrenatalRecordFunction = () => {
      ...
      if (patientRecordInfo.value.midwife_id.user_id != null) {
        patientRecordInfo.value.midwife_id = patientRecordInfo.value.midwife_id.user_id
      }
    ...
    const closePrescription = () => {
      ...
      if (
        ...
        (prenatal_checkup.value.prescription[prenatal_checkup.value.prescription.length - 1]
          .medicine_name === '' ||
          prenatal_checkup.value.prescription[prenatal_checkup.value.prescription.length - 1]
            .quantity === '')
      ) {
